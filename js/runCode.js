...
function runCode() {
   // console.log(elem.context);
			...
			function getScript(i) {
				if(scripts[i] != undefined) {
					$.get(scripts[i], function(d) {
						...
						saveWorker.postMessage({editorText: editor.getValue(), gifData: gifData, scriptTxt:scriptText, title:title});
		...
		function codeCopy() {
            ...
            worker.postMessage({code: copiedCode});
		}
